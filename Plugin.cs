...
namespace RavenM
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            {
                var badPath = customBuildInMutators;
                customBuildInMutators = "NOT_REAL";
                Logger.LogError($"Directory {badPath} could not be found.");
            }
        ...
        void Update()
        {
            ...
            if (!FirstSteamworksInit)
            {
                ...
                // Инициализируем LAN компоненты
                var lanTransportObject = new GameObject();
                lanTransportObject.AddComponent<LanTransport>();
                DontDestroyOnLoad(lanTransportObject);

                var lanLobbyObject = new GameObject();
                lanLobbyObject.AddComponent<LanLobbySystem>();
                DontDestroyOnLoad(lanLobbyObject);

                var lanUIObject = new GameObject();
                lanUIObject.AddComponent<LanLobbyUI>();
                DontDestroyOnLoad(lanUIObject);

                Logger.LogInfo("LAN компоненты инициализированы");
            }
