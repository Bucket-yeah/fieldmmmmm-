...
namespace RavenM
{
    ...
    public class OnStartPatch
    {
        static bool Prefix()
        {
            // LAN host: bind game socket and notify all clients
            if (LanLobbySystem.instance != null && LanLobbySystem.instance.IsHostingLobby)
            {
                OptionsPatch.SetConfigValues(false);
                // Set name-tag flags before scene loads (GameUI.Awake reads them)
                var _lanInfo = LanLobbySystem.instance.GetCurrentLobbyInfo();
                if (_lanInfo != null)
                {
                    LobbySystem.instance.nameTagsEnabled      = true;
                    LobbySystem.instance.nameTagsForTeamOnly  = _lanInfo.NameTagsForTeamOnly;
                }
                IngameNetManager.instance.OpenRelayLan();
                LanLobbySystem.instance.BroadcastGameStart();
                return true;
            }
    ...
    public class FinalizeStartPatch
    {
        ...
        static void Postfix()
        {
            // LAN mode: bypass Steam transport entirely
            if (LanLobbySystem.instance != null && LanLobbySystem.instance.IsInLobby)
            {
                if (LanLobbySystem.instance.IsHostingLobby)
                {
                    IngameNetManager.instance.StartAsServerLan();
                }
                else
                {
                    var info = LanLobbySystem.instance.GetCurrentLobbyInfo();
                    if (info != null && info.HostIP != null)
                    {
                        var hostEP = new System.Net.IPEndPoint(info.HostIP, IngameNetManager.LAN_GAME_PORT);
                        IngameNetManager.instance.StartAsClientLan(hostEP);
                    }
                }
                return;
            }
