...
namespace RavenM
{
    ...
    public class OnExitGamePatch
    {
        static void Postfix()
        {
            ...
            if (!IngameNetManager.instance.LanMode)
            {
    ...
    public class IngameNetManager : MonoBehaviour
    {
        ...
        // ── LAN mode (offline UDP transport) ─────────────────────────────
        public bool LanMode = false;
        public IPEndPoint LanHostEndPoint;
        public Dictionary<string, IPEndPoint> LanClients = new Dictionary<string, IPEndPoint>();
        public Dictionary<string, Guid> LanEndpointGuidMap = new Dictionary<string, Guid>();

        private struct LanInbound { public IPEndPoint Src; public byte[] Data; }
        private UdpClient _lanGameSocket;
        public const int LAN_GAME_PORT = 7780;
        private bool _lanRunning = false;
        private Thread _lanReceiveThread;
        private readonly Queue<LanInbound> _lanInboundQueue = new Queue<LanInbound>();
        private readonly object _lanInboundLock = new object();
        private readonly Queue<byte[]> _lanLoopbackQueue = new Queue<byte[]>();
        private readonly object _lanLoopbackLock = new object();
        // ─────────────────────────────────────────────────────────────────
        ...
        private void OnGUI()
        {
            ...
            {
                GUI.Label(new Rect(10, 80, 200, 40), "LAN mode");
            }
        ...
        {
            if (LanLobbySystem.instance != null && LanLobbySystem.instance.IsHostingLobby)
            {
                OpenRelayLan();
                return;
            }
        ...
        // ── LAN transport methods ─────────────────────────────────────────

        public void OpenRelayLan()
        {
            Plugin.logger.LogInfo("LAN: Opening relay on port " + LAN_GAME_PORT);
            LanMode = true;
            IsHost = true;
            ServerConnections.Clear();
            ConnectionGuidMap.Clear();
            GuidActorOwnership.Clear();
            LanClients.Clear();
            LanEndpointGuidMap.Clear();
            lock (_lanInboundLock) _lanInboundQueue.Clear();
            lock (_lanLoopbackLock) _lanLoopbackQueue.Clear();

            try
            {
                _lanGameSocket = new UdpClient(LAN_GAME_PORT) { EnableBroadcast = false };
                _lanRunning = true;
                _lanReceiveThread = new Thread(LanReceiveLoop) { IsBackground = true, Name = "LAN-GameRX" };
                _lanReceiveThread.Start();
                Plugin.logger.LogInfo("LAN relay socket bound on port " + LAN_GAME_PORT);
            }
            catch (Exception ex)
            {
                Plugin.logger.LogError("LAN relay socket failed: " + ex.Message);
                LanMode = false;
                IsHost = false;
            }
        }

        public void StartAsServerLan()
        {
            Plugin.logger.LogInfo("LAN: StartAsServer.");
            LanMode  = true;
            IsHost   = true;
            IsClient = true;

            // Apply own team from lobby selection (already set on IAM dropdown before StartGame)
            if (LanLobbySystem.instance != null)
            {
                var _info = LanLobbySystem.instance.GetCurrentLobbyInfo();
                LobbySystem.instance.nameTagsEnabled     = true;
                LobbySystem.instance.nameTagsForTeamOnly = _info?.NameTagsForTeamOnly ?? true;
            }
        ...
        public void StartAsClientLan(IPEndPoint hostEndPoint)
        {
            Plugin.logger.LogInfo("LAN: StartAsClient. Host=" + hostEndPoint);
            LanMode         = true;
            IsClient        = true;
            LanHostEndPoint = hostEndPoint;

            // Apply name-tag settings from lobby info
            if (LanLobbySystem.instance != null)
            {
                var _info = LanLobbySystem.instance.GetCurrentLobbyInfo();
                LobbySystem.instance.nameTagsEnabled     = true;
            ...
            var player = ActorManager.instance.player;
            int pid = RandomGen.Next(13337, int.MaxValue);
            player.gameObject.AddComponent<GuidComponent>().guid = pid;
            ClientActors.Add(pid, player);
            OwnedActors.Add(pid);

            try
            {
                _lanGameSocket = new UdpClient(0) { EnableBroadcast = false };
                _lanRunning = true;
                _lanReceiveThread = new Thread(LanReceiveLoop) { IsBackground = true, Name = "LAN-GameRX" };
                _lanReceiveThread.Start();
                Plugin.logger.LogInfo("LAN client socket bound, sending to " + hostEndPoint);
            }
            catch (Exception ex)
            {
                Plugin.logger.LogError("LAN client socket failed: " + ex.Message);
            }
        }
        ...
        private void LanReceiveLoop()
        {
            ...
            while (_lanRunning && _lanGameSocket != null)
            {
                try
                {
                    EndPoint remote = new IPEndPoint(IPAddress.Any, 0);
                    int size = _lanGameSocket.Client.ReceiveFrom(buf, ref remote);
                    if (size > 0)
                    {
                        byte[] copy = new byte[size];
                        Buffer.BlockCopy(buf, 0, copy, 0, size);
                        lock (_lanInboundLock)
                            _lanInboundQueue.Enqueue(new LanInbound { Src = (IPEndPoint)remote, Data = copy });
                    }
                }
                catch (SocketException) when (!_lanRunning) { break; }
                catch (Exception ex)
                {
                    if (_lanRunning)
                        Plugin.logger.LogWarning("LAN RX error: " + ex.Message);
                }
            }
        }

        private void StopLanTransport()
        {
            if (!_lanRunning && _lanGameSocket == null) return;
            _lanRunning = false;
            try { _lanGameSocket?.Close(); } catch { }
            _lanGameSocket = null;
            if (_lanReceiveThread != null && _lanReceiveThread.IsAlive)
                _lanReceiveThread.Join(300);
            _lanReceiveThread = null;
            LanClients.Clear();
            LanEndpointGuidMap.Clear();
            lock (_lanInboundLock) _lanInboundQueue.Clear();
            lock (_lanLoopbackLock) _lanLoopbackQueue.Clear();
            LanMode = false;
            LanHostEndPoint = null;
        }
        ...
        public void SendPacketToServer(byte[] data, PacketType type, int send_flags)
        {
            ...
            if (LanMode)
            {
                if (IsHost)
                {
                    List<string> _dead = null;
                    foreach (var _ck in LanClients.Keys.ToList())
                    {
                        try { _lanGameSocket.Send(packet_data, packet_data.Length, LanClients[_ck]); }
                        catch (Exception ex)
                        {
                            Plugin.logger.LogWarning($"LAN send to {_ck} failed: {ex.Message}");
                            if (_dead == null) _dead = new List<string>();
                            _dead.Add(_ck);
                        }
                    }
                    if (_dead != null) foreach (var dk in _dead) LanClients.Remove(dk);
                }
                else
                {
                    try { _lanGameSocket.Send(packet_data, packet_data.Length, LanHostEndPoint); }
                    catch (Exception ex) { Plugin.logger.LogError("LAN send to host failed: " + ex.Message); }
                }
            }
            else
            {
        ...
        void FixedUpdate()
        {
            ...
            if (IsClient)
            {
                // Collect messages: LAN queue or Steam
                var _pending = new List<byte[]>();
                if (LanMode)
                {
                    if (IsHost)
                    {
                        lock (_lanLoopbackLock)
                        { while (_lanLoopbackQueue.Count > 0) _pending.Add(_lanLoopbackQueue.Dequeue()); }
                    }
                    else
                    {
                        lock (_lanInboundLock)
                        { while (_lanInboundQueue.Count > 0) { var _p = _lanInboundQueue.Dequeue(); _pending.Add(_p.Data); } }
                    }
                }
                else
                {
                    ...
                    for (int _si = 0; _si < msg_count; _si++)
                    {
                        var _sm = Marshal.PtrToStructure<SteamNetworkingMessage_t>(msg_ptr[_si]);
                        var _sd = new byte[_sm.m_cbSize];
                        Marshal.Copy(_sm.m_pData, _sd, 0, _sm.m_cbSize);
                        _pending.Add(_sd);
                        SteamAPI_SteamNetworkingMessage_t_Release.Invoke(null, new object[] { msg_ptr[_si] });
                    }
                }
                foreach (var msg_data in _pending)
                {
            ...
            if (IsHost)
            {
                if (LanMode)
                {
                    ...
                    lock (_lanInboundLock)
                    {
                        _inbound = _lanInboundQueue.ToArray();
                        _lanInboundQueue.Clear();
                    }
                    foreach (var _pkt in _inbound)
                    {
                        string _srcKey = _pkt.Src.ToString();
                        if (!LanClients.ContainsKey(_srcKey))
                        {
                            LanClients[_srcKey] = _pkt.Src;
                            try
                            {
                                using var _ms = new MemoryStream(_pkt.Data);
                                using var _rdr = new ProtocolReader(_ms);
                                _rdr.ReadInt32();
                                var _g = new Guid(_rdr.ReadBytes(16));
                                LanEndpointGuidMap[_srcKey] = _g;
                                if (!GuidActorOwnership.ContainsKey(_g))
                                    GuidActorOwnership[_g] = new List<int>();
                                Plugin.logger.LogInfo($"LAN: client connected from {_pkt.Src} GUID={_g}");
                            }
                            catch (Exception ex) { Plugin.logger.LogWarning("LAN GUID read error: " + ex.Message); }
                        }
                        List<string> _dead = null;
                        foreach (var _ck in LanClients.Keys.ToList())
                        {
                            if (_ck == _srcKey) continue;
                            try { _lanGameSocket.Send(_pkt.Data, _pkt.Data.Length, LanClients[_ck]); }
                            catch (Exception ex)
                            {
                                Plugin.logger.LogWarning($"LAN relay to {_ck} failed: {ex.Message}");
                                if (_dead == null) _dead = new List<string>();
                                _dead.Add(_ck);
                            }
                        }
                        if (_dead != null) foreach (var dk in _dead) LanClients.Remove(dk);
                        lock (_lanLoopbackLock) _lanLoopbackQueue.Enqueue(_pkt.Data);
                    }
                }
                else
                {
